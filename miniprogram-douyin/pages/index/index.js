...
Page({
  ...
  async handleCheckin() {
    if (this.data.checkedIn) {
      tt.showToast({ title: '今日已签到', icon: 'none' })
      return
    }
    ...
    try {
      ...
      if (result && result.success) {
        ...
        tt.showToast({ title: `签到成功 +${result.points_added}积分`, icon: 'success' })
        this.loadData()
      }
    } catch (err) {
      tt.showToast({ title: '签到失败，请重试', icon: 'none' })
    }
  },

  goTongue() {
    tt.switchTab({ url: '/pages/tongue/tongue' })
  },

  goPoints() {
    tt.switchTab({ url: '/pages/points/points' })
  },

  goGuarantee() {
    tt.navigateTo({ url: '/pages/guarantee/guarantee' })
  }
})
